...
class FairPostProcessor:
    ...
    def fit(self, probs, y_true, sensitive_attr):
        ...
        idx = self.selector.select(self.pareto_front_, self.metric_directions_)
        self.best_thresholds_ = self.threshold_history_[idx]
        self.best_metrics_ = self.metric_history_[idx]
        self.best_losses_ = self.loss_history_[idx]
